...
namespace LeaRun.Application.Web.Areas.PerfScheme.Controllers
{
    ...
    public class PerfSchemeSettingController : MvcControllerBase
    {
        ...
        /// <returns></returns>
        public ActionResult SaveSchemeDep(string year, string fabh, string jgfamc, string tableIds, string KPI, string jgfabh = null)
        {
            if (year.IsEmpty())
            {
                return Error("请选择一个方案");
            }
            List<Dictionary<string, string>> KPIIDS = null;
            if (!string.IsNullOrEmpty(KPI))
            {
                KPIIDS = JsonConvert.DeserializeObject<List<Dictionary<string, string>>>(KPI);
            }
            var entities = new List<BpePA003Entity>();
            var tbIds = tableIds.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var id in tbIds)
            {
                var entity = new BpePA003Entity { JXBM = year, FABH = fabh, JGFAMC = jgfamc, STATUS = "0", JGBM = id };
                entities.Add(entity);
            }
            bll.SaveSchemeDepList(fabh, year, jgfabh, tableIds, entities, KPIIDS);
