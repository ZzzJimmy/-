...
namespace LeaRun.Application.Busines.PerfScheme
{
    ...
    public class PerfSchemeSettingBLL
    {
        ...
        #region 提交数据
        public void SaveSchemeDepList(string fabh, string jxbm, string jgfabh, string jgbms, List<BpePA003Entity> entities, List<Dictionary<string, string>> kpis)
        {
            var delFabh = ibpePA003Service.SaveSchemeDepList(fabh, jxbm, jgfabh, jgbms, entities);
            List<BpePA004Entity> kpiEntities = new List<BpePA004Entity>();
            foreach (var e in entities)
            {
                if (kpis != null)
                {
                    kpiEntities.AddRange(kpis.Select(k => new BpePA004Entity
                    {
                        JGFABH = e.JGFABH,
                        KPIBH = k["id"],
                        ZBLX = k["zblx"],
                ...
                {
                    var baseEntities = ibpePA002Service.GetList(fabh);

                    kpiEntities.AddRange(baseEntities.Select(b => new BpePA004Entity
                    {
                        JGFABH = e.JGFABH,
                        KPIBH = b.KPIBH,
                        ZBLX = b.ZBLX,
            ...
            ibpePA004Service.SaveSchemeDepDetails(kpiEntities, delFabh);
        }
        /// <summary>
        /// 删除对象绩效方案
        /// </summary>
        ...
        public void DelDepPerfSchemeKPI(string jgfabh)
        {
            try
            {
                ibpePA002Service.DelDataByFABH(jgfabh);
            }
            catch (Exception)
            {
